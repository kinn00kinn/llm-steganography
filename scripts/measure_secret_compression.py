...
def generate_secrets(count: int) -> list[str]:
    ...
    actions = [
        "カフェで本を読み", "図書館で勉強し", "家で映画を見て",
        "公園を散歩し", "オンラインで会議をして"
    ]
    thoughts = [
        "とてもリラックスできた", "少し疲れたが充実していた",
        "新しい発見があって驚いた", "次回もまた挑戦したい",
        "次はもっと上手くやれると思う"
    ]
    details = [
    ...
    closings = ["明日はもっと早く起きよう。", "今日はゆっくり休むことにする。", "来週の発表に向けて準備を進めたい。", "週末が待ち遠しい。", "充実した一日だった。"]  # noqa: E501
...
def main():  # type: ignore
    ...
    def report(name, arr):  # type: ignore
        ...
        mx = max(arr)
        print(f"{name:10s}: mean={mean:6.1f}  median={median:6.1f}  p90={p90:6.1f}  max={mx:6.1f} (bits)")  # noqa: E501
