...
def generate_secrets(count: int) -> list[str]:
    ...
    actions = ["研究室に行き", "カフェでコーヒーを飲み", "図書館で勉強し", "家で映画を見て", "公園を散歩し", "オンラインで会議をして"]
    thoughts = ["とても有意義な時間だった", "少し疲れたが充実していた", "新しい発見があって驚いた", "次回もまた挑戦したい", "次はもっと上手くやれると思う"]
    details = [
    ...
    closings = ["明日はもっと早く起きよう。", "今日はゆっくり休むことにする。", "来週の発表に向けて準備を進めたい。", "週末が待ち遠しい。", "充実した一日だった。"]

    secrets = []
    for i in range(count):
        # build a text of roughly 100 characters
        text = f"{random.choice(subjects)}は今日、{random.choice(actions)}、{random.choice(thoughts)}。{random.choice(details)}{random.choice(closings)}"
        # pad or truncate to exact 100 chars? 
        # The requirement says "100文字の代表的な秘密文". Let's just add random padding words if it's too short.
        while len(text) < 100:
...
def compute_lm_bits(backend: TransformersBackend, text: str) -> float:
    ...
    # To compute this, we need to evaluate the probability of each token given the previous tokens.
    # We can do this efficiently by passing the full sequence through the model in one forward pass
    # but TransformersBackend only provides next_logits.
    # For Phase 5C, we can just use the internal _model directly for speed, or call next_logits repeatedly.
    # Calling next_logits repeatedly is slow for 100 tokens * 100 samples (10000 passes).
    # Since this is a measurement script, bypassing the interface for speed is acceptable if we document it.
    
    # We will use the internal _model for a single forward pass per sequence to compute all logits at once.
    token_ids = backend.tokenize(text)
    if not token_ids:
        return 0.0

    # Add a BOS/pad if necessary. Qwen usually doesn't need BOS, but we need a starting context.
    # Actually, the probability of the first token p(t_0) is unconditioned or conditioned on some BOS.
    ...
    # As a reasonable approximation, we just prepend a known token (e.g., newline or space) to represent context.
    # Let's prepend the Qwen special token or just evaluate from t_1 given t_0, and add a fixed cost for t_0.
...
def main():
    ...
    def report(name, arr):
        ...
        mx = max(arr)
        print(f"{name:10s}: mean={mean:6.1f}  median={median:6.1f}  p90={p90:6.1f}  max={mx:6.1f} (bits)")

    print(f"\n--- Compression Results for 100-character Japanese secrets (N={args.samples}) ---")
    report("UTF-8 Raw", raw_bits)
    report("zlib", zlib_bits)
    report("bz2", bz2_bits)
    report("lzma", lzma_bits)
    report("LM (Qwen)", lm_bits)
