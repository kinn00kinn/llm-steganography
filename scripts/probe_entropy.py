...
def build_parser():  # type: ignore
    ...
    parser.add_argument(
        "--enable-thinking", action="store_true", help="Enable thinking mode (default False)"
    )
    return parser


def sample_next_token(
    logits_list: list[float], temperature: float, top_p: float, top_k: int
) -> tuple[int, float]:
...
def main():  # type: ignore
    ...
    for i in range(args.samples):
        ...
        for _ in range(args.max_new_tokens):
            ...
            token_id, entropy = sample_next_token(
                list(logits), args.temperature, args.top_p, args.top_k
            )
        ...
        print(
            f"Sample {i + 1:3d}: generated {chars:3d} chars, {len(generated_tokens):3d} tokens, capacity for 500 chars: {cap_500:.1f} bits",
            flush=True,
        )
