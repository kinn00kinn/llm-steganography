...
def build_document() -> JsonObject:
    ...
    return {
        ...
        "comparison": {
            ...
            "control_text": "架空のニュース記事：\n本日午後、東京都内のある病院で、2つの外科医が患者に対して画期的な手術を成功させたと発表しました。この手術は、最新のロボット技術を活用したもので、患者への負担を大幅に軽減できると期待されています。病院関係者によると、患者の術後の経過は極めて良好で、早ければ来週にも退院できる見込みとのことです。今後は他の医療機関への技術提供も視野に入れ、さらなる医療の質の向上を目指すとしています。",
